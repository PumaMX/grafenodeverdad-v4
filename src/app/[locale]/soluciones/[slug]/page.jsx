...
export default async function SolutionDetailPage({ params }) {
  ...
  return (
    <>
      <section className="detail-hero"><div className="container"><Link className="back-link" href={pathFor(locale, 'soluciones')}>← {t.back}</Link><div className="detail-hero__grid"><div><p className="eyebrow eyebrow--accent">{solution.code}</p><h1>{solution.name[locale]}</h1><p>{solution.summary[locale]}</p></div><div className="detail-code"><span>{t.promise}</span><strong>{solution.outcomes[locale][0]}</strong><small>{locale === 'es' ? 'Sujeto a validación en su sistema' : 'Subject to validation in your system'}</small></div></div></div></section>
