...
export const solution = defineType({
  ...
  title: 'Solución',
  type: 'document',
  groups: [{ name: 'content', title: 'Contenido', default: true }, { name: 'development', title: 'Ficha de aplicación' }, { name: 'media', title: 'Imágenes' }, { name: 'seo', title: 'SEO' }],
  fields: [
    defineField({ name: 'name', title: 'Nombre', type: 'localizedString', group: 'content', validation: bilingualRequired }),
    defineField({ name: 'slug', title: 'Ruta', type: 'slug', options: { source: 'name.es' }, group: 'content', validation: (Rule) => Rule.required() }),
    ...
    defineField({
      ...
      type: 'solutionApplicationProfile',
      group: 'development',
      description: 'Ficha pública de madurez, evidencia, sectores, piloto y referencias. No incluya recetas, formulaciones detalladas ni condiciones confidenciales.',
    }),
    defineField({
      name: 'projects',
      title: 'Proyectos y rutas de aplicación',
      type: 'array',
      description: 'Agregue, ordene o retire proyectos dentro de esta familia. El estado editorial evita presentar una línea planteada como producto validado.',
      of: [defineArrayMember({ type: 'solutionProject' })],
      group: 'content',
    }),
    defineField({ name: 'leadImage', title: 'Imagen principal', type: 'editorialImage', group: 'media' }),
    defineField({ name: 'gallery', title: 'Galería', type: 'array', of: [defineArrayMember({ type: 'editorialImage' })], group: 'media' }),
